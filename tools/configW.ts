...
export const PROD_DESTW            = `${DIST_DIR}/prodw`;
...
// Declare the asset files which must be injected here
export const APP_ASSETS: InjectableDependency[] = [
  { src: `${ASSETS_SRC}/main.css`, inject: true, dest: CSS_DEST },
  { src: `${ASSETS_SRC}/**/*.css`, inject: true, dest: CSS_DEST }
];
...
function getEnvironment() {
  ...
  let prodKeywordW = !!base.filter(o => o.indexOf(ENVIRONMENTS.PRODUCTIONW) >= 0).pop();
  if (base && prodKeywordW || argv['env'] === ENVIRONMENTS.PRODUCTIONW) {
    util.log('ENV: ', chalk.red(ENVIRONMENTS.PRODUCTIONW));
    return ENVIRONMENTS.PRODUCTIONW;
  } else if (base && prodKeyword || argv['env'] === ENVIRONMENTS.PRODUCTION) {
    util.log('ENV: ',chalk.red(ENVIRONMENTS.PRODUCTION));
      return ENVIRONMENTS.PRODUCTION;
  } else {
      util.log('ENV: ',chalk.red(ENVIRONMENTS.DEVELOPMENT));
